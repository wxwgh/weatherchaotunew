<template>
	<div class="rightpanel_div_parent" :class="{'left_div':get_right_config.type==='浮空平台轨迹'}">
		<div class="rightpanel_div" v-if="get_right_flag">
			...
			<div class="rightpanel_parent">
				...
				<!-- 内容 -->
				<div class="rightpanel_main" v-if="get_right_config.type==='人员单位信息'">
					<div class="rightpanel_main_header">
						<span>{{get_right_config.title2}}</span>
					</div>
					<div class="rightpanel_main_content">
						<div v-for="(value, key, index) in get_attribution2">
							<span>{{key}}：</span>
							<span>{{value}}</span>
						</div>
					</div>
				...
				<div class="rightpanel_main" v-else-if="get_right_config.type==='人员装备信息'">
					<div class="rightpanel_main_header">
						<span>{{get_right_config.title}}</span>
					</div>
					<div class="rightpanel_main_content">
				...
				<div class="rightpanel_main" v-else-if="get_right_config.type==='浮空平台轨迹'">
					<div class="rightpanel_main_header">
						<span>{{get_right_config.title}}</span>
					</div>
					<div class="rightpanel_main_content">
					...
					<div class="rightpanel_time_parent">
						<span>时间范围</span>
						<el-date-picker v-model="uav_track_time" popper-class="time_select_class_uav" align="left"
							size="mini" type="datetimerange" range-separator="至" start-placeholder="开始时间"
							end-placeholder="结束时间">
						</el-date-picker>
					</div>
					...
					<div class="scope_list_parent">
						<div class="rightpanel_main_header">
							<span>历史影像</span>
						</div>
						<div class="scope_result_parent"><!--post.is_active-->
							<div v-for="post in get_uav_record" :class="{'scope_result':true,'active':post.is_active}" @click="uav_record_list_click(post)">
								<div>
									<span style="width:22px"/>
									<span>{{post.name}}</span>
								</div>
								<div>
									<span>创建时间：{{post.time}}</span>
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="rightpanel_main" v-else-if="get_right_config.type==='浮空平台火场态势'">
					<div class="rightpanel_main_header">
						<span>{{get_right_config.title}}</span>
					</div>
					<div class="rightpanel_main_content">
				...
				<div class="rightpanel_main" v-else-if="get_right_config.type==='火场'">
					<div class="rightpanel_main_header" v-show="!get_result_flag">
						<span>{{get_right_config.title}}</span>
					</div>
					<div class="rightpanel_main_content" v-show="!get_result_flag">
						<div v-for="(value, key, index) in get_attribution">
							<span>{{key}}：</span>
							<span>{{value}}</span>
						</div>
				...
				<div class="rightpanel_main" v-else-if="get_right_config.type==='森林防火资源'">
					<div class="rightpanel_main_header">
						<span>{{get_right_config.title}}</span>
					</div>
					<div class="rightpanel_main_content">
	...
	export default {
		...
		computed: {
			...
			get_attribution: function() {
				if (this.$store.state.right_panel_config.type === "人员单位信息") {
			...
			get_attribution2: function() {
				return this.$store.state.right_panel_config.attribution_danwei_zhudi;
			},
			...
			get_uav_record: function() {
				return this.$store.state.uav_history_record;
			}
		},
		methods: {
			//输入提示查询
			...
			//浮空平台历史影像点击
			uav_record_list_click(post){
				if(this.$store.state.task_config.track_uavrecord_line_layer!=""){
					this.$store.state.task_config.track_uavrecord_line_layer.remove();
				}
				if(this.$store.state.task_config.track_uavrecord_amerker_layer!=""){
					this.$store.state.task_config.track_uavrecord_amerker_layer.remove();
				}
				var map = this.$store.state.map;
				//判断列表是否选中
				if(post.is_active){
					...
					if(this.last_post != ""){
						this.last_post.is_active = false;
					}
					post.is_active=true;
					this.axios({
						method:"put",
						url:"http://18.76.1.191:10081/uav/"+post.id+"/playback/1"
					}).then(function(result){
						
					})
					this.last_post = post;

					if(this.last_uav_path_datas != "" && this.last_uav_path_datas.length != 0)
					{

						var datas = this.last_uav_path_datas;
						//console.log(post);
						//console.log(this.last_uav_path_datas);

						var points = [];

						var starttime = post.time.substr(0,19).replaceAll("-","").replaceAll("T","").replaceAll(":","");
						var endtime = starttime.substr(0,8)+240000;

						for (let i = 0; i < datas.length; i++) {
							if(points.length == 0 && datas[i].createDate > starttime)
							{
								points.push([datas[i-1].latitude, datas[i-1].longtitude]);
								points.push([datas[i].latitude, datas[i].longtitude]);
							}
							if(points.length > 0 && datas[i].createDate > starttime && datas[i].createDate < endtime)
							{
								points.push([datas[i].latitude, datas[i].longtitude]);
							}
							if(points.length > 0 && datas[i].createDate > endtime)
							{
								points.push([datas[i].latitude, datas[i].longtitude]);
								break;
							}
						}
						this.$store.state.task_config.track_uavrecord_line_layer = L.polyline(points, {
								color: "red",
								weight: 5
						}).addTo(map);
						...
						var carIcon = L.icon({
							iconSize: [40, 40],
							iconAnchor: [20, 20],
							iconUrl: require("../assets/images/uav/feiji.png")
						})
						// 动态marker
						this.$store.state.task_config.track_uavrecord_amerker_layer = L.animatedMarker(this.$store.state.task_config.track_uavrecord_line_layer.getLatLngs(), {
							speedList: speedList,
							interval: 200, // 默认为100mm
							icon: carIcon
						}).addTo(map)

						this.$store.state.task_config.track_uavrecord_amerker_layer.start();
			...
			uav_track(){
				...
				if (this.uav_track_time == "" || this.uav_track_time == null) {
					...
					this.axios({
						...
						$this.last_uav_path_datas = datas;
						//点数组
						var points = [];
						if (datas != null && datas.length > 0) {
							...
							for (let i = 0; i < datas.length; i++) {
								if (i == 0) {
									$this.myCommon.create_marker([datas[i].latitude, datas[i].longtitude],
										right_config.start_icon_url, datas[i], "浮空平台轨迹查询", "");
								} else if (i == datas.length - 1) {
									$this.myCommon.create_marker([datas[i].latitude, datas[i].longtitude],
										right_config.end_icon_url, datas[i], "浮空平台轨迹查询", "");
								} else {
									/* 
									$this.myCommon.create_marker([datas[i].latitude, datas[i].longtitude],
										right_config.mid_icon_url, datas[i], "浮空平台轨迹查询", "");
									*/
								}
							}
							

							map.fitBounds($this.$store.state.task_config.track_line_layer.getBounds(),{ padding: [20, 20] ,maxZoom:14});
					...
					this.axios({
						...
						$this.$store.state.uav_history_record = datas;
						//$this.$store.state.loading = false;
					})
				}
			},
						...
						if (datas != null && datas.length > 0) {
							for (let i = 0; i < datas.length; i++) {
								if (i == 0) {
									...
									$this.myCommon.create_marker([datas[i].latitude, datas[i].longitude],
										right_config.mid_icon_url, datas[i], "人员轨迹查询节点", "");
								}
							...
							map.fitBounds($this.$store.state.task_config.track_line_layer.getBounds(),{ padding: [20, 20] });
					...
					this.axios({
						...
						if (datas != null && datas.length > 0) {
							for (let i = 0; i < datas.length; i++) {
								if (i == 0) {
									$this.myCommon.create_marker([datas[i].latitude, datas[i].longtitude],
										right_config.start_icon_url, datas[i], "人员轨迹查询本地", "");
								} else if (i == datas.length - 1) {
									$this.myCommon.create_marker([datas[i].latitude, datas[i].longtitude],
										right_config.end_icon_url, datas[i], "人员轨迹查询本地", "");
								} else {
									$this.myCommon.create_marker([datas[i].latitude, datas[i].longtitude],
										right_config.mid_icon_url, datas[i], "人员轨迹查询节点本地", "");
								}
								points.push([datas[i].latitude, datas[i].longtitude]);
							}
							//创建折线路径
