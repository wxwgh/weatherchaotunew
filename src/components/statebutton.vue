	...
	export default {
		...
		computed:{
			get_button_config:function(){
				var temp_config = this.$store.state.bottom_panel_config;
		...
		methods: {
			button_click(post) {
				var $this = this;
				var config = this.$store.state.bottom_panel_config;
				var type = post.type;
				// 设置子菜单选中或取消
				...
				this.axios({
					...
					if(temp_datas.length<=0){
						$this.myCommon.get_message("今日无在线设备");
					}else{
						for(let i=0;i<temp_datas.length;i++){
							var temp_config=$this.get_right_config(temp_datas[i],type,post.name);
							$this.myCommon.create_marker([temp_config.attribution.纬度,temp_config.attribution.经度],post.icon_url,temp_config,"在线人员位置底部","");
						}
					}
			...
			get_right_config(data,type,child_name){
				var $this =this;
				var temp_config={};
				var left_configs = $this.$store.state.bottom_panel_config;
