		...
		methods: {
			...
			child_click(post) {
				...
				if (type === "人员单位信息") {
					...
					this.$store.state.ryzbquery_flag = true;
					var params = {
					...
					this.axios({
						...
						if($this.$store.state.dbw_config.length>0){
							...
							$this.myCommon.get_message("短报文暂无数据");
						}
					...
					// 	var temp_config = $this.get_right_config(datas[i], type,post.name);
					// 	$this.myCommon.create_marker([temp_config.attribution.纬度, temp_config.attribution.经度],
					// 		post.icon_url, temp_config, "浮空平台轨迹","");
					// }
					this.axios({
						method: "get",
						url: post.url,
					...
					this.axios({
						...
						for (let i = 0; i < datas.length; i++) {
							...
							$this.myCommon.create_marker([temp_config.attribution.边际点纬度, temp_config.attribution
								.边际点经度
							], post.icon_url, temp_config, "浮空平台火场态势","");
						}
					})
					...
					if(this.$store.state.currentwxyg != "" && this.$store.state.currentwxyg.is_active)
					{
						this.$store.state.currentwxyg.is_active = false;

						// 清除marker
						if (this.$store.state.currentwxyg.markers) {
							for (let s = 0; s < this.$store.state.currentwxyg.markers.length; s++) {
								this.$store.state.currentwxyg.markers[s].remove();
							}
							this.$store.state.currentwxyg.markers = [];
						}
					}

					this.$store.state.currentwxyg = post;
					...
					this.axios({
						...
						var datas = result.data.data;
						if(datas.length<=0){
							$this.myCommon.get_message("当前时间段内无热点信息");
						}else{
							for (let i = 0; i < datas.length; i++) {
								var temp_config = $this.get_right_config(datas[i], type,post.name);
								$this.myCommon.create_marker([temp_config.attribution.纬度, temp_config.attribution.经度],
					...
					this.axios({
						...
						if(res.data.data.length<=0){
							$this.myCommon.get_message("暂无数据");
						}else{
			...
			get_right_config(data, type,child_name) {
				...
				if (type === "人员单位信息") {
					...
					for (let key in temp_config.attribution_danwei_zhudi) {
						temp_config.attribution_danwei_zhudi[key] = data[0][key];
					}
				} else if (type === "单位驻地信息") {
					temp_config.name = $this.is_null(data[0].总队) + $this.is_null(data[0].支队) + $this.is_null(data[0].大中队)
