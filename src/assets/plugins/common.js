...
export default{
	...
	create_marker(latlng,icon_url,temp_config,name,type){
		...
		if(name==="遂行任务"){
			...
			marker = L.marker(latlng, {icon: myIcon});
			marker.setZIndexOffset(10000);
			marker.bindTooltip(temp_config.positime,{
				direction:"right",
				sticky:true,
				offset:L.point(5,0)
			})
			task_config.track_marker_layer.addLayer(marker);
		}else if(name==="浮空平台轨迹查询"){
			var myIcon = L.icon({
			    iconUrl: icon_url,
				iconSize: [30, 39],
				iconAnchor: [15, 39],
		...
		}else if(name==="人员单位信息"||name==="单位驻地信息"){
			var myIcon = L.icon({
			    iconUrl: icon_url,
				iconSize: [44, 44],
				iconAnchor: [22, 44],
			...
			marker.on("click",function(){
				...
				$store.state.right_panel_config=this.attribution;
				$store.state.right_panel_flag=true;
				$store.state.uav_video_flag=true;

			})
			$this.push_markers(temp_config.type,temp_config.child_name,marker);
		}else if(name==="浮空平台火场态势"){
			var myIcon = L.icon({
			    iconUrl: icon_url,
				iconSize: [44, 39],
				iconAnchor: [22, 39],
			...
			$this.push_markers3(temp_config.type,temp_config.child_name,marker);
		}else if(name==="森林防火资源"){
			var myIcon = L.icon({
			    iconUrl: icon_url,
				iconSize: [21, 21],
			...
			$this.push_markers2(temp_config.type,temp_config.child_name,marker);
		}
		return marker;
	},
	//填充marker数组
	...
	push_markers3(type,name,marker){
		var left_configs = $store.state.bottom_panel_config;
		for(let i=0;i<left_configs.length;i++){
			for(let j=0;j<left_configs[i].child.length;j++){
				if(type===left_configs[i].child[j].name){
					for(let x=0;x<left_configs[i].child[j].child.length;x++){
	...
	//森林你防火资源
	push_markers2(type,name,marker){
		var left_configs = $store.state.slfhzy;
		for(let x=0;x<left_configs.child.length;x++){
			if(name === left_configs.child[x].name){
				left_configs.child[x].markers.push(marker);
			}
		}
	},
	get_message(message){
	...
	clear_track_marker_layer(){
		...
		if($store.state.task_config.track_uavrecord_line_layer!=""){
			$store.state.task_config.track_uavrecord_line_layer.remove();
		}
		if($store.state.task_config.track_uavrecord_amerker_layer!=""){
			$store.state.task_config.track_uavrecord_amerker_layer.remove();
		}
	},
	//根据名称获取right_config
